...
const approveTransaction = async (req, res) => {
  try {
    ...
    if (req.query.newSchool) {
      ...
      await finalTransaction.update({ _schoolId: newSchool._id });
    }
...
const approveDeniedTransaction = async (req, res) => {
  try {
    ...
    if (req.query.newSchool) {
      ...
      await transaction.update({ _schoolId: newSchool._id });
    }
    return res.status(200).json({ status: 'Record approved' });
  } catch (err) {
    console.log(err);
...
/**
 * Wipe all denied transactions at given location.
 * @param {Object} req - Request object.
 * @param {Object} res - Response object.
 * */
const clearDeniedTransactions = async (req, res) => {
  try {
    await Transaction.destroy({
      where: { status: 2, _locationId: req.location._id },
    });
    return res.json(req.transaction);
  } catch (err) {
    console.log(err);
